...
public sealed class AssessmentApiTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Steps_catalogue_is_ordered_and_complete()
    {
        ...
        Assert.Equal(["entity", "segment", "verification", "screening", "website", "prohibited", "mcc", "match", "presence", "owners", "licensing", "bank", "financials", "plausibility", "credit", "terms", "score", "case"], ids);
    }
    ...
    public async Task Run_completes_every_step_keeps_failed_sources_as_gaps_and_persists_result_with_pdf()
    {
        ...
        Assert.Equal("Succeeded", steps["entity"]);
        Assert.Equal("Succeeded", steps["segment"]);
    ...
    public async Task Stream_emits_catalogue_step_events_then_result()
    {
        ...
        Assert.Equal(18, stepEvents.Count(s => s.GetProperty("status").GetString() is "Succeeded" or "Failed" or "Skipped"));
