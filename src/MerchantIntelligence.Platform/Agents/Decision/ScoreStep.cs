...
public sealed class ScoreStep(UnifiedRiskScorer scorer, RulesEngine rules, RuleSetRepository ruleSets) : IAssessmentStep
{
    public WorkflowStepDescriptor Descriptor { get; } = new("score", "Unified risk score & policy rules",
        ...
        ["verification", "presence", "screening", "website", "prohibited", "mcc", "match", "owners", "licensing", "bank", "financials", "plausibility", "credit", "terms"],
        ["verification", "presence", "screening", "website", "prohibited", "mcc", "match", "owners", "licensing", "bank", "financials", "plausibility", "credit", "terms"], true, []);
    ...
    public async Task ExecuteAsync(AssessmentContext ctx)
    {
        ...
        ctx.Signals = AssessmentComposer.CollectSignals(v, s, ctx.Website, ctx.Prohibited, ctx.Mcc, ctx.Bank, ctx.Financials, ctx.Plausibility, ctx.Owners, ctx.BankEvidence, ctx.Licensing);
