...
## Deploying (Docker / Render free tier)

The `Dockerfile` builds the Angular app and the API into one image; the API serves the SPA from
`wwwroot` (any non-`/api`, non-`/swagger` route falls back to `index.html`) and listens on `$PORT`
(default 8080).

```bash
docker build -t merchant-intelligence .
docker run --rm -p 8080:8080 merchant-intelligence   # http://localhost:8080  (UI, /swagger, /health)
```
...
- there is no persistent disk, so the SQLite platform DB (`Platform__DatabasePath`) and the sanctions
  cache reset on each deploy/restart — attach a Render disk (paid) or point the platform store at an
  external database for durable cases/audit;
- 512 MB RAM; the image idles at ~50 MB and ~170 MB with the MCC classifier loaded.
